...
def alerts(park_name = ""):
    if(park_name == ""):
        ...
        json_response = alerts_query(park_name)
        titles = []
        description = []
        category = []
        if(int(json_response['total'])==0):
            return render_template("alerts.html",string=("Currently no Alerts for " + park_name),no_center=True)
        else:
...
def visitor_center_query(park_code):
    ...
    output = []
    if(int(json_response['total'])==0):
        output.append("No Visitor Centers Specified for this Park")
    else:
        for i in range(0,int(json_response['total'])):
            output.append(json_response['data'][i]['name'])
    return output

def alerts_query(park_name):
